...
* **`authentik-server-info` (Server $\rightarrow$ Outpost)**: Exposes the core server's API URL and bootstrap API token (`AUTHENTIK_BOOTSTRAP_TOKEN`) to the LDAP Outpost, enabling user queries and authorization policy checks over secure HTTP.
