# receive-ca-cert Specification

## Purpose
The `receive-ca-cert` specification defines how the Authentik Server Operator imports, merges, and propagates custom Certificate Authority (CA) certificates. This ensures that both the host-level subprocesses (e.g., standard Python HTTP clients) and the workload container processes (e.g., Authentik server) trust secure endpoints signed by custom or private CAs.
